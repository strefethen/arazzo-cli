...
- **Files** — exact paths and current line numbers (verified at `e5dfede`; re-grep
  before editing since line numbers drift — see the per-item reconciliation notes
  on items 2/3/7 where lines have already moved on `main`)
...
> --check` were all clean; `cargo test --workspace` failed **only** on the three
> tests addressed by item 1.
>
> **Reconciliation (2026-06-24):** `main` is now at `d0015f1`, 9 commits ahead of
> the baseline. Build/clippy/fmt/test were **not** re-run as part of this
> reconciliation. The remediation *code* has not been started; the only plan item
> with progress on `main` is item 2 (tags `v0.1.0`–`v0.2.2` now exist and
> release-readiness tooling landed). The three live-httpbin tests in item 1 are
> still present and non-hermetic.
...
> **Reconciliation (2026-06-24):** Partially addressed on `main`. Tags
> `v0.1.0`–`v0.2.2` now exist — the "**No git tags exist at all**" claim in *Why*
> below is obsolete — and release-readiness tooling landed
> (`scripts/release/verify-readiness.sh`, `docs/internal-release.md`; commit
> `4b58cd8`). **Still open:** `CHANGELOG.md` has no `0.2.x` sections, and the
> `--step`/`--no-deps` bullets remain misfiled under `0.1.0`.
...
> **Reconciliation (2026-06-24):** Outstanding, unchanged on `main`, but line
> numbers have drifted. The merge bug is now at
> `crates/arazzo-validate/src/lib.rs:691-694` (was `668-671`); `OnAction.type_` is
> still a plain `ActionType` at `crates/arazzo-spec/src/lib.rs:576`. Re-grep all
> call sites in the *Change* section below before editing.
...
> **Reconciliation (2026-06-24):** Outstanding on `main` — the client stubs
> (`vscode-arazzo-debug/src/yamlStepIndex.ts`, `breakpointMapper.ts`) are still
> present and `src/test/smoke.test.ts` is still `assert.equal(true, true)`.
> **Drift:** the DAP adapter was split into focused modules since the baseline
> (commit `fc65043`), so every `crates/arazzo-debug-adapter/src/dap.rs:NNNN`
> reference in 7a (e.g. `resolve_source_breakpoints` at `dap.rs:1034`,
> `setBreakpoints` at `dap.rs:317`) is **stale** — re-locate the server-side
> anchors across the new modules before relying on them.
