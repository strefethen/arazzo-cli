...
async fn invalid_simple_success_criterion_fails_closed() {
    ...
    let error = match new_test_engine(&server.base_url, spec)
        .execute_collect("invalid-success-criterion", BTreeMap::new())
        .await
        .outputs
    {
        Ok(_) => panic!("invalid success criterion must fail the step"),
        Err(error) => error,
    };
...
#[tokio::test]
async fn invalid_simple_action_criterion_is_nonmatching() {
    let paths = Arc::new(Mutex::new(Vec::<String>::new()));
    let paths_ref = Arc::clone(&paths);
    let server = start_server(move |_method, path, _headers, _body| {
        paths_ref
            .lock()
            .unwrap_or_else(|_| panic!("recording request paths"))
            .push(path.clone());
        if path == "/fail" {
    ...
    let spec = make_spec(vec![Workflow {
        workflow_id: "invalid-action-criterion".to_string(),
        steps: vec![
            Step {
                step_id: "request".to_string(),
                ...
                on_failure: vec![
                    OnAction {
                        ...
                        criteria: vec![SuccessCriterion {
                            condition: "$statusCode contains 500".to_string(),
                            ..SuccessCriterion::default()
                        }],
                        ..OnAction::default()
                    },
    ...
    let result = new_test_engine(&server.base_url, spec)
        .execute_collect("invalid-action-criterion", BTreeMap::new())
        .await
        .outputs;

    assert!(
        result.is_ok(),
        "valid later action must handle failure: {result:?}"
    );
    assert_eq!(
        paths
            .lock()
...
#[tokio::test]
async fn valid_simple_criterion_runs_in_nested_workflow() {
    let server = start_server(|_method, _url, _headers, _body| MockHttpResponse::empty(200));
    let spec = make_spec(vec![
        Workflow {
            workflow_id: "child".to_string(),
            steps: vec![Step {
                step_id: "child-request".to_string(),
    ...
    assert!(
        result.is_ok(),
        "nested simple criterion must pass: {result:?}"
    );
}
