...
fn first_enum_value<T: Clone>(enumeration: &[Option<T>]) -> Option<T> {
    enumeration.iter().find_map(|v| v.clone())
}

fn generate_integer_example(i: &openapiv3::IntegerType) -> Value {
...
fn generate_number_example(n: &openapiv3::NumberType) -> Value {
...
fn generate_string_example(field_name: &str, string_type: &openapiv3::StringType) -> Value {
...
fn field_name_heuristic(lower: &str) -> Option<String> {
...
"#;
    ...
    fn test_string_field_name_email_format_wins() {
        ...
        // Format should win over field-name heuristic for "phone".
        let result = generate_string_example("phone", &s);
